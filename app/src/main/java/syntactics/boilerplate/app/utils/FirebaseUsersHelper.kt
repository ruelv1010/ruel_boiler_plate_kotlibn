...
class FirebaseUsersHelper {
    ...
    fun addTodoItem(myUsersModel: MyUsersModel, onSuccess: () -> Unit, onFailure: (Exception) -> Unit) {
        // Generate a unique key or use a more robust ID generation
        val todoId = database.child("Users").push().key
            ?: run {
        ...
        val todoItem = myUsersModel.copy(id = todoId)

        database.child("Users").child(todoId).setValue(todoItem)
